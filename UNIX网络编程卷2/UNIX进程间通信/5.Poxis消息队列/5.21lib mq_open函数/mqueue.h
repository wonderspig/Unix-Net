...
struct mymq_hdr {
  ...
  long				mqh_head;	/* index of first message 当前队列中的第一个消息指针索引 */
  long				mqh_free;	/* index of first free message 当前队列中的第一个空消息指针索引 */
  long				mqh_nwait;	/* #threads blocked in mq_receive() 有多少个线程阻塞在mq_receive上 */
  pid_t				mqh_pid;	/* nonzero PID if mqh_event set 如果设置消息队列非空就通知进程,此处为待通知的进程pid(消息方式的通知) */
  struct sigevent	mqh_event;	/* for mq_notify() 消息队列非空通知进程的具体信息 */
