...
get_ifi_info(int family, int doaliases)
{
	...
	ifipnext = &ifihead; //链表的下一个节点指针指向头节点指针
	lastname[0] = 0;
	sdlname = NULL;
/* end get_ifi_info1 */
...
#ifdef	HAVE_SOCKADDR_DL_STRUCT //如果定义了HAVE_SOCKADDR_DL_STRUCT,系统支持返回AF_LINK地址族的sockaddr结构
...
/* include get_ifi_info3 */
		ifi = Calloc(0, sizeof(struct ifi_info)); //分配一个大小为一个ifi_info结构体的内存空间,每一位都置1
		//ifipnext是二级指针,是指向"一个节点的指针"的指针,所以*ifipnext就是"一个节点的指针"
		//所以如果把ifipnext定义为指向"下一个节点的指针"的指针,那么*ifipnext就是就应该指向"当前节点的指针"
		//把当前指针放在放在链表的末尾
		*ifipnext = ifi;			/* prev points to this new one */
		...
		/* If the sockaddr_dl is from a different interface, ignore it 
		   判断sockaddr_dl结构的实现是否如预期不如预期就忽略它 */
...
/* include get_ifi_info4 */
		...
		case AF_INET: //IP4地址族
			...
			ifi->ifi_addr = Calloc(0, sizeof(struct sockaddr_in)); //动态分配存放地址的空间,初始化为0
			...
			if (flags & IFF_BROADCAST) { //接口标志带有广播标志
				...
				ifi->ifi_brdaddr = Calloc(0, sizeof(struct sockaddr_in)); //为广播地址分配存储空间,初始化为0
			...
			if (flags & IFF_POINTOPOINT) { //带有点到点链路地址的标志
				...
				ifi->ifi_dstaddr = Calloc(0, sizeof(struct sockaddr_in)); //分配点到点链路地址存储空间,初始化为0
		...
		case AF_INET6: //IP6地址族
			...
			ifi->ifi_addr = Calloc(0, sizeof(struct sockaddr_in6)); //动态分配存放地址的空间,初始化为0
