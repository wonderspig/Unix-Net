...
main(int argc, char **argv)
{
	...
	socklen_t len;
	size_t rd_sz;
	struct sctp_initmsg initm; //这个结构体为套接字选项SCTP_INITMSG所使用的结构体

/* include modified_sctpserv02 */
	if (argc == 2)
		stream_increment = atoi(argv[1]);
        sock_fd = Socket(AF_INET, SOCK_SEQPACKET, IPPROTO_SCTP);
	bzero(&initm,sizeof(initm)); //清空并初始化initm结构体
	initm.sinit_num_ostreams = SERV_MORE_STRMS_SCTP; //把结构体中请求外出流的数目设定为需要的值
	Setsockopt(sock_fd, IPPROTO_SCTP, SCTP_INITMSG,
		   &initm, sizeof(initm)); //设置套接字选项
